...
import com.example.a02travel.model.HistoryModel;
import com.example.a02travel.adapter.HistoryAdapter;
...
public class HistoryActivity extends AppCompatActivity {
    ...
    public void refreshList() {
        ...
        csr = db.rawQuery("SELECT * FROM TB_HOTEL WHERE emailUsr_htl = '" + email + "'", null);
        csr.moveToFirst();
        for (int i = 0; i < csr.getCount(); i++){
            csr.moveToPosition(i);
            nmHotel = csr.getString(2);
            totalHrg = csr.getString(7);
            tglHtl = csr.getString(3);
            id_hotel = csr.getString(1);
            riwayat = "Berhasil melakukan booking " + nmHotel + ".";
            hasil.add(new HistoryModel(id_hotel, tglHtl, riwayat, totalHrg, R.drawable.profile));
        }
